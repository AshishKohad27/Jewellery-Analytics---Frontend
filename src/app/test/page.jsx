  ...
  return (
    <div className="p-4 lg:p-8 space-y-8">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        <h2 className="text-lg font-semibold text-slate-800 mb-4">Counter Test</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100">
          ...
          <p className="text-2xl font-bold text-slate-800">{paramsData?.total?.items || data.length}</p>
        </div>
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100">
          <p className="text-sm text-slate-500 mb-1">Total Pages</p>
          <p className="text-2xl font-bold text-slate-800">{paramsData?.total?.pages || 0}</p>
        </div>
        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100">
          <p className="text-sm text-slate-500 mb-1">Roles</p>
          <p className="text-2xl font-bold text-slate-800">{roleStats.labels.length}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-slate-800">Users by Role</h3>
            <p className="text-sm text-slate-500">Distribution of users across roles</p>
          </div>
          <div className="h-64">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-slate-800">User Count per Role</h3>
            ...
            <tbody className="divide-y divide-slate-100">
              {data.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50">
                  <td className="px-6 py-4 text-sm text-slate-800">{user.id}</td>
                  <td className="px-6 py-4 text-sm text-slate-700">{user.name}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{user.email}</td>
                  {/* Role badge with global role colors */}
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getRoleColor(user.role?.name).bg} ${getRoleColor(user.role?.name).text}`}>
                      {user.role?.name}
                    </span>
                  </td>
                  {/* Status badge */}
